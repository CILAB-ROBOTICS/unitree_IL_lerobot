...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        # Optionally replace the ACT encoder with weights from an externally trained encoder
    ...
    def replace_encoder_from_checkpoint(self, checkpoint_path: str | Path, encoder_prefix: str | None = None):
        ...
        This is useful when pretraining the encoder (e.g., using a head camera on a tactile carpet)
        and wanting to reuse those weights end-to-end inside ACT.
        Args:
            checkpoint_path: Path to a checkpoint that contains the pretrained encoder weights.
            encoder_prefix: Optional prefix indicating where the encoder weights live inside the checkpoint's state dict (e.g., "model.encoder"). 
            If not provided, common prefixes are tried in order.
        """
