    ...
    try:
        ...
        if user_input.lower() == "s":
            ...
            while True:
                ...
                # Depth
                depth_image = tv_img_array.copy() # mod for depth
                depth_tensor = torch.from_numpy(depth_image)
                # [H, W, C] -> [C, H, W]
                if depth_tensor.ndim == 3 and depth_tensor.shape[-1] == 3:
                    depth_tensor = depth_tensor.permute(2, 0, 1).contiguous()

                observation["observation.depths.cam_left_high"] = depth_tensor

                print("observation keys:", observation.keys())
                print("depth shape:", observation["observation.depths.cam_left_high"].shape)
